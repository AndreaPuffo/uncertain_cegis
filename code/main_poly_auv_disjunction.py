...
while not found_lyap:
    ...
    res = direct(min_hurwi_eig, bounds=bounds, args=(P_star, K), eps=1.)
    print(f'Max eigenvalue of variable lyapunov matrix: {-res.fun}')
    A_found, Bs_found = closest_vertex(res.x, lb_ab, ub_ab)
    print(f'Max eigenvalue of closed loop matrix with lyapunov (vertex): '
          f'{np.max(np.linalg.eigvals(A_found + Bs_found[0] @ K))}, '
          f'{np.max(np.linalg.eigvals(A_found + Bs_found[1] @ K))}, '
          f'{np.max(np.linalg.eigvals(A_found + Bs_found[2] @ K))}')
    ...
    if res.fun <= 0:
        print('found cex')
        A_cex, Bs_cex = closest_vertex(res.x, lb_ab, ub_ab)
        sampled_A += [A_cex]
        sampled_B += [Bs_cex]
    else:
        print('valid Lyap')
