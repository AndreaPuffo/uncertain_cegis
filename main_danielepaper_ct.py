...
while not found_lyap:
    ...
    res = direct(min_hurwi_eig, bounds=bounds, args=(P_star, K))
    print(f'Max eigenvalue of variable lyapunov matrix: {-res.fun}')
    A_found = res.x[:4 * 4].reshape(4, 4)
    B_found = res.x[4 * 4:].reshape(4, 1)
    print(f'Max eigenvalue of closed loop matrix with lyapunov (DiRect): {np.max(np.linalg.eigvals(A_found + B_found @ K))}')
    A_found_vtx, B_found_vtx = closest_vertex(res.x, [A_min.reshape(1, -1)[0], B_min.reshape(1, -1)[0]],
                                              [A_max.reshape(1, -1)[0], B_max.reshape(1, -1)[0]])
    print(f'Max eigenvalue of closed loop matrix with lyapunov (Closest Vtx): '
          f'{np.max(np.linalg.eigvals(A_found_vtx + B_found_vtx @ K))}')
